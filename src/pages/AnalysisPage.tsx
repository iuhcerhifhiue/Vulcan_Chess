...
const AnalysisPage: React.FC<AnalysisPageProps> = ({ username }) => {
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      const data: AnalysisResults | { error: string } = await res.json();

      if (!res.ok) {
        throw new Error((data as { error: string }).error || 'Analysis failed');
      }

      setAnalysis(data as AnalysisResults);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-8">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Action Button */}
        {!analysis && !isAnalyzing && ( // Only show button if not analyzing and no analysis results yet
          <button
